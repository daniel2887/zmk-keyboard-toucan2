...
static void draw_level_peripheral_left(lv_obj_t *canvas, const struct status_state *state) {
    lv_draw_img_dsc_t img_dsc_l;
    lv_draw_img_dsc_init(&img_dsc_l);

    uint8_t level = state->battery_p;
#if !IS_ENABLED(CONFIG_ZMK_SPLIT_ROLE_CENTRAL)
    // When compiled on the peripheral half, status_state.battery contains
    // the peripheral's battery level.
    level = state->battery;
#endif
...
void draw_battery_peripheral_left_status(lv_obj_t *canvas, const struct status_state *state) {
    draw_level_peripheral_left(canvas, state);
}
